...
def predict() -> None:
    ...
    with open('model/fin_model.pkl', 'rb') as file:
    ...
    pred_proba = pd.DataFrame({'proba': model.predict_proba(X)[:, 1].T})
    res = pred_proba['proba'].apply(lambda x: 0 if x < 0.03 else 1)

    dfr = pd.DataFrame(zip(X.index, res))
    dfr.columns = [['id', 'predict']]

    dfr.to_csv('predictions/sber_predict.csv', index=False)

    print(round(roc_auc_score(y, res), 4))
